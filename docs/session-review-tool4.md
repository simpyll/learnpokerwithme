...
/* ── HEADER ── */
#hdr{display:flex;align-items:center;gap:7px;padding:0 10px;height:44px;flex-shrink:0;background:rgba(6,14,26,0.99);border-bottom:1px solid var(--border);z-index:200;position:relative}
...
#h-stakes{background:rgba(77,138,255,0.12);border:1px solid rgba(77,138,255,0.25);border-radius:20px;padding:2px 8px;font-size:10px;font-family:'IBM Plex Mono',monospace;color:var(--accent);display:none;flex-shrink:0}
#h-profit{font-size:11px;font-family:'IBM Plex Mono',monospace;font-weight:600;display:none;padding:2px 8px;border-radius:20px;flex-shrink:0}
...
.btn-load{background:linear-gradient(135deg,#2a48aa,#5030a8);color:#fff;padding:6px 11px;font-size:11px}
.btn-export{background:linear-gradient(135deg,#0a6855,#0b9070);color:#fff;padding:6px 11px;font-size:11px;display:none}
.btn-filter{background:rgba(245,197,66,0.1);border:1px solid rgba(245,197,66,0.28);color:var(--gold);padding:5px 9px;font-size:10px;display:none;border-radius:5px}
...
@media(max-width:680px){#mbtn{display:flex}}
...
/* ── ACTION TOAST (floats above active player, auto-fades) ── */
.act-toast{
...
.act-toast.fading{animation:toastOut 0.4s ease forwards}
@keyframes toastOut{to{opacity:0;transform:translate(-50%,-60%) scale(0.9)}}
...
<div id="hdr">
  ...
  <div class="h-sp"></div>
  <button class="btn btn-filter" id="filtbtn" onclick="toggleFilter()">⭐ Big Hands</button>
  <button class="btn btn-export" id="expbtn" onclick="exportPost()">📝 Export</button>
  <button class="btn btn-load" onclick="document.getElementById('fi').click()">📂 Load</button>
...
// ════════════════════════════════════════════
// ACTION TOAST SYSTEM
// Shows a brief coloured toast above each player
// Fades out after ~1.5s, cleared on new street
// ════════════════════════════════════════════
const TOAST_COLORS={
  folds:'#374151',checks:'#1a3560',calls:'#0f4020',
  bets:'#5a3300',raises:'#5a1010',wins:'#35145a',collects:'#35145a',shows:'#0e2a40'
};
const TOAST_TC={folds:'#8090a0',checks:'#6090e0',calls:'#40c070',bets:'#f0a030',raises:'#ff6060',wins:'#c090ff',collects:'#c090ff'};

// Store: playerName -> {el, timerId}
let activeToasts={};

function showActionToast(playerName, action, slotIdx, nPlayers){
  // Remove existing toast for this player
  clearToast(playerName);

  const slots=getSlots(nPlayers);
  const s=slots[slotIdx]||slots[0];

  // Toast appears ~40px ABOVE the player chip
  // We use slightly different y offset to not overlap cards
  const topPct=(s.top*100-16)+'%';
  const leftPct=(s.left*100)+'%';

  const bg=TOAST_COLORS[action.type]||'#2a3a50';
  const tc=TOAST_TC[action.type]||'#d0e0f0';
  const hand=VIEW[hi];
  const bbAmt=action.amount>0?` ${(action.amount/hand.bb).toFixed(1)}BB`:'';
  const label=action.type.toUpperCase()+bbAmt;

  const el=document.createElement('div');
  el.className='act-toast';
  el.style.cssText=`top:${topPct};left:${leftPct};background:${bg};color:${tc};border-color:${tc}33`;
  el.textContent=label;
  document.getElementById('pw').appendChild(el);

  // Auto-fade after 1.4s
  const tid=setTimeout(()=>{
    if(el.parentNode){el.classList.add('fading');setTimeout(()=>el.parentNode&&el.parentNode.removeChild(el),400);}
    delete activeToasts[playerName];
  },1400);
  activeToasts[playerName]={el,tid};
}

function clearToast(playerName){
  if(activeToasts[playerName]){
    clearTimeout(activeToasts[playerName].tid);
    const el=activeToasts[playerName].el;
    if(el.parentNode)el.parentNode.removeChild(el);
    delete activeToasts[playerName];
  }
}
function clearAllToasts(){
  Object.keys(activeToasts).forEach(clearToast);
}
...
function render(){
  ...
  // Clear toasts on new street or new hand
  if(st.frame?.t==='deal'||st.frame?.t==='board'){clearAllToasts();}
  ...
  // Preserve existing toast divs, rebuild player boxes
  const toastEls=[...pwEl.querySelectorAll('.act-toast')];
  pwEl.innerHTML='';
  // Re-add toasts
  toastEls.forEach(el=>pwEl.appendChild(el));

  hand.players.forEach(p=>{
    const slot=sm[p.position]??0;
    const pos=slots[slot]||slots[0];
    const ia=st.active===p.name,ih=p.isHero,ib=p.seat===hand.button;
    ...
    if(p.cards)html+=`<div class="pcards">${p.cards.map(c=>cardH(c,'sm',false)).join('')}</div>`;
    else if(!hf)html+=`<div class="pcards">${cardH(null,'sm',true)}${cardH(null,'sm',true)}</div>`;
    let cc='pchip'+(ih?' hero':'')+(ia?' active':'')+(hf?' fold':'');
    html+=`<div class="${cc}"><span class="ppos${ih?' hero':''}">${dn}</span><span class="pstk">${stBB}BB</span></div>`;
    if(ia)html+=`<div class="tarrow">▼</div>`;
  ...
  // Show toast for current action frame
  if(st.frame?.t==='action'){
    const a=st.frame.a;
    const p=hand.players.find(x=>x.name===a.player);
    if(p){
      const slot=sm[p.position]??0;
      showActionToast(p.name,a,slot,n);
    }
  }
  ...
  const f=st.frame;let el='';
  if(f){
    if(f.t==='deal')el='🃏 Cards Dealt';
    else if(f.t==='board'){const cards=(f.c||[f.c2]).join(' ');el=`🎴 ${f.s.toUpperCase()}: ${cards}`;}
    else if(f.t==='action'){const p=hand.players.find(x=>x.name===f.a.player),pn=p?p.position+(p.isHero?' (You)':''):'?',ba=f.a.amount>0?` ${(f.a.amount/hand.bb).toFixed(1)}BB`:'';el=`${pn} ${f.a.type}${ba}`;}
    else if(f.t==='result')el=hand.winners.length?`🏆 ${hand.winners.map(w=>{const p=hand.players.find(x=>x.name===w.name);return(p?p.position+(p.isHero?' (You)':''):'?')+' wins '+(w.amount/hand.bb).toFixed(1)+'BB';}).join(', ')}`:'🏁 Complete';
  }
  document.getElementById('ib-e').textContent=el;
...
.atst{position:absolute;transform:translate(-50%,-50%);background:rgba(4,8,18,0.92);border:1px solid rgba(255,255,255,0.12);border-radius:8px;padding:4px 9px;font-size:11px;font-weight:700;color:#fff;white-space:nowrap;pointer-events:none;z-index:40;animation:tIn 0.15s ease forwards}
@keyframes tIn{from{opacity:0;transform:translate(-50%,-50%) scale(0.8)}to{opacity:1;transform:translate(-50%,-50%) scale(1)}}
.atst.fd{animation:tOut 0.4s ease forwards}@keyframes tOut{to{opacity:0;transform:translate(-50%,-60%) scale(0.9)}}
...
.ch4.act{border-color:var(--gold)!important;box-shadow:0 0 0 2px rgba(245,197,66,0.25),0 0 16px rgba(245,197,66,0.3);background:rgba(35,25,4,0.95)!important;animation:gl 0.9s ease infinite}
@keyframes gl{0%,100%{box-shadow:0 0 0 1px rgba(245,197,66,0.25),0 0 10px rgba(245,197,66,0.18)}50%{box-shadow:0 0 0 2px rgba(245,197,66,0.5),0 0 20px rgba(245,197,66,0.38)}}
.ch4.fld{opacity:0.3}
...
.ta4{position:absolute;bottom:-13px;left:50%;transform:translateX(-50%);font-size:9px;color:var(--gold);animation:bb 0.5s ease infinite alternate}
@keyframes bb{from{transform:translateX(-50%) translateY(0)}to{transform:translateX(-50%) translateY(-3px)}}
...
const TC2={folds:'#374151',checks:'#1a3560',calls:'#0f4020',bets:'#5a3300',raises:'#5a1010',wins:'#35145a',collects:'#35145a'};
const TC3={folds:'#8090a0',checks:'#6090e0',calls:'#40c070',bets:'#f0a030',raises:'#ff6060',wins:'#c090ff',collects:'#c090ff'};
...
let AH2=HH2,VH2=[...HH2],hi2=0,fi2=0,fr2=[],pl2=false,sp2=2200,iv2=null,fa2=false,at2={};
...
function clrT(n){if(at2[n]){clearTimeout(at2[n].t);const e=at2[n].el;if(e.parentNode)e.parentNode.removeChild(e);delete at2[n];}}
function clrAll(){Object.keys(at2).forEach(clrT);}
function shwT(pn,a,sl,n){clrT(pn);const slots=gSl(n),s=slots[sl]||slots[0],h=VH2[hi2],top=(s.t*100-16)+'%',left=(s.l*100)+'%';const bg=TC2[a.type]||'#2a3a50',tc=TC3[a.type]||'#d0e0f0',ba=a.amount>0?' '+(a.amount/h.bb).toFixed(1)+'BB':'',lbl=a.type.toUpperCase()+ba;const el=document.createElement('div');el.className='atst';el.style.cssText=\`top:\${top};left:\${left};background:\${bg};color:\${tc};border-color:\${tc}33\`;el.textContent=lbl;document.getElementById('pw2').appendChild(el);const tid=setTimeout(()=>{if(el.parentNode){el.classList.add('fd');setTimeout(()=>el.parentNode&&el.parentNode.removeChild(el),400);}delete at2[pn];},1400);at2[pn]={el,t:tid};}
...
if(st.f?.t==='deal'||st.f?.t==='board')clrAll();
...
const pw=document.getElementById('pw2');const te=[...pw.querySelectorAll('.atst')];pw.innerHTML='';te.forEach(e=>pw.appendChild(e));
h.players.forEach(p=>{const sl=sm[p.position]??0,pos=slots[sl]||slots[0],ia=st.act===p.name,ih=p.isHero,ib=p.seat===h.button,hf=st.fd.has(p.name),isN=h.newP&&h.newP.includes(p.name),stBB=(p.stack/h.bb).toFixed(0),dn=p.position+(ih?' (You)':'');let html='';if(ib)html+=\`<div class="db4">D</div>\`;if(isN)html+=\`<div class="nb4">NEW</div>\`;if(p.cards)html+=\`<div class="pc4">\${p.cards.map(c=>cH4(c,'sm',false)).join('')}</div>\`;else if(!hf)html+=\`<div class="pc4">\${cH4(null,'sm',true)}\${cH4(null,'sm',true)}</div>\`;html+=\`<div class="ch4\${ih?' hero':''}\${ia?' act':''}\${hf?' fld':''}"><span class="pp4\${ih?' h':''}">\${dn}</span><span class="ps4">\${stBB}BB</span></div>\`;if(ia)html+=\`<div class="ta4">▼</div>\`;const d=document.createElement('div');d.className='pb4';d.style.cssText=\`position:absolute;top:\${pos.t*100}%;left:\${pos.l*100}%;transform:translate(-50%,-50%)\`;d.innerHTML=html;pw.appendChild(d);});
if(st.f?.t==='action'){const a=st.f.a,p=h.players.find(x=>x.name===a.player);if(p)shwT(p.name,a,sm[p.position]??0,n);}
