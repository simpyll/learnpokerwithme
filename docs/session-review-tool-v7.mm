...
// acts: persist across ALL streets (show last action taken all hand)
// bets: clear per street (chips on table reset each street)
// folded: persist whole hand
// ════════════════════════════════════════════
function getState(){
  ...
  for(let i=0;i<=fi;i++){
    ...
    if(f.t==='deal'){
      ...
      // New street — only clear bet chips, keep action labels
      Object.keys(bets).forEach(k=>delete bets[k]);
      ...
      const a=f.a;
      acts[a.player]=a;  // always update to latest action
      if(a.type==='folds'){
        folded.add(a.player);
...
function loadH(idx){
  ...
  frames=buildFrames(VIEW[hi]);fi=0;
